...
Current FAQ version:  0.99


=head1 DESCRIPTION

This is version  0.98 of the PDL FAQ, a collection of  frequently 
...
from someones posting the authorship should be indicated, let
the FAQ maintainer know if it isn't. For more general
information explicit acknowledgment is not made in the text,
but rather there is an incomplete list of contributors at the
end of this document. Please contact the FAQ maintainer if
you feel hard done by.



Send your comments, additions, suggestions or corrections to the PDL
mailing list at perldl@jach.hawaii.edu. See Q: 3.2 below for instructions 
...
requires version 5.8 of perl, or higher.  PDL may build with
perl 5.6.2 if the F<Makefile.PL> is edited to change the
C<use 5.8.0> to C<use 5.6.2> in the eval check.  However,
pre-5.8 perls are not supported.  More information on where
and how to get a Perl installation can be found at the Perl
home page L<http://www.perl.org> and at many CPAN sites
(if you do not know what I<CPAN> is, check the answer to
the next question).

To build PDL you also need a working C compiler, support for Xsubs,
and the package Extutils::MakeMaker. If you don't have a compiler
there might be a binary distribution available, see
"Binary distributions" below.
...
C<pdl2> shell type C<help> .  Using the C<help> and 
...
line. To illustrate this process, here is the record of a typical pdl2
...
As of this writing (FAQ version 0.99 of 07 April 2011) the latest stable version
is 2.4.8.  The latest stable version should always be available from a CPAN
...
purposes and more general availability.  The current post-PDL-2.4.7 developers
release of PDL is available as F<CHM/PDL-2.4.7_011.tar.gz> .
...
To install PDL in a non-standard location, use the PREFIX and LIB
options in the C<perl Makefile.PL> configure step.  For example,
C<perl Makefile.PL PREFIX=/myprefix LIB=/myprefix/lib/perl5> will
configure PDL to install the Perl code into the /myprefix/lib/perl5
and the system install files like binary executables in the
directories C</myprefix/bin, /myprefix/lib, /myprefix/include...>.
...
though they might not be the most recent version. If someone is 
interested in providing binary distributions for other 
...
might not always be the latest version.  PDL-2.4.8 builds out of
the box on Strawberry Perl for Windows.
...
Git version control system (see L<http://git-scm.com>).  Note that
some auxiliary project repositories (the project website, some books,
etc.) are still available via CVS.
...
With versions of Perl prior to 5.6.7 this has to be done using
...
In Perl 5.6.7 and higher this assignment can be simplified
...
see L<PDL::Lvalue> for more info.  PDL now supports a more
...
problem PDL has built-in support to extent the PDL class via the 
I<has-a> relation for blessed hashes. You can get the I<HAS-A> behave 
like I<IS-A> simply in that you assign the PDL
...
FITS I/O is implemented by the C<wfits>/C<rfits> functions in L<PDL::IO::Misc> .
...
L<PDL::IO::Pic> implements an interface to the NetPBM/PBM+ filters to
...
released version of PDL 2.4.7 [DH, please confirm].
