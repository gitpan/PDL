...
This is version 0.4 of the PDL FAQ, a collection of
...
You can find the latest version of this document at F<http://www.aao.gov.au/local/www/kgb/perldl/faq.html>.
...
know. Send your comments to the PDL mailing list at F<perldl@jach.hawaii.edu>
(preferably) or to the FAQ maintainer Christian Soeller (F<csoelle@sghms.ac.uk>).
...
sections are marked with the strings '[!beta!]' or '[!alpha!]'. These
reflect development that is under way and will hopefully soon
culminate in the release of PDL 2.0. Right now we are quite confident
about the stability of the latest alpha versions (1.9XXX) so that we
suggest you had a look at those versions if you need features which
are not yet supported in the released versions (see below for directions
on how to get alpha distributions).

New (B<[+]>) and changed (B<[!]>) questions and answers in this release are
marked.
...
5.004 of Perl, version 5.004 or higher is
...
F<http://www.aao.gov.au/local/www/kgb/perldl>. There are currently no other mirror sites in other parts of the world.
...
F<http://www.aao.gov.au/local/www/kgb/perldl/ports.html>. If you don't have a compiler you can check if a
...
the TODO list) at the PDL home site located at F<http://www.aao.gov.au/local/www/kgb/perldl>.
...
be accessed by pointing your web browser to F<http://www.aao.gov.au/local/www/kgb/perldl>. It has
...
As of this writing (FAQ version 0.4 of 29.7.97) the
latest released version is 1.11. Currently in alpha
test is 1.93_03. For those of you who are really
...
F<http://www.aao.gov.au/local/www/kgb/perldl/alpha.html>.

=head2 I am looking for a package to do XXX in PDL. Where shall I look for it?

A good place to start is again F<http://www.aao.gov.au/local/www/kgb/perldl>. We hope to get round
...
=head2 B<[!]> [!alpha!] There is this great XXX package on the net. Has it already been interfaced to PDL or how can I do it?

Check on PDL's home site F<http://www.aao.gov.au/local/www/kgb/perldl> if the package in question
...
More specifically, it says (L<PDL>):
...
C<at>/C<sec> (see L<PDL> manpage) or in the new alpha versions [!alpha!] the
powerful C<slice> function and friends (see L<PDL::Indexing>).
...
However, if you want to know, have a look at L<PDL::Dataflow> 
in the current alpha distribution. There are applications which will benefit
from this feature (and it is already at work behind the scenes in the
alpha versions).
...
documentation (see L<PDL> manpage):
...
=head2 B<[!]> What happens when I have several references to the same PDL object in different variables (cloning, etc?) ?
...
of it just create a copy explicitly:
...
=head2 B<[+]> [!alpha!] What I/O formats are supported by PDL ?
...
released version of PDL 1.11. Some minor modifications
are required if you want to use this module with the current alpha
versions.
...
=head2 B<[+]> [!alpha!] What is a C<null> pdl ?
...
=head2 B<[+]> [!alpha!] What is the I<signature> of a PDL function ?

The I<signature> of a function is an important concept in PDL.
Many (but not all) PDL functions have a I<signature>
...
facility. For details check the L<PDL::Indexing> manpage.

=head2  What is a piddle C<(;)> ?

Well, PDL scalar variables (which are instances of a particular class
of perl objects, i.e. blessed thingies (see L<perlfaq>)) are
...
prettypodder and various other improvements.
