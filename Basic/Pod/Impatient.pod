...
of these (but not all as sometimes it is not appropriate) follow
...
There is also an interactive shell, C<perldl>, see L<perldl>.
...
=head2 Arithmetic
 ...
 $f = index($a, where($a > 10)); # where returns set of indices for 1D pdl.
...
The most important are L<PDL::Primitive> and <PDL::Slices>.
...
Of course, this function is already available as the C<inner> 
...
See L<PDL::Graphics::TriD> (the name sucks...). this is a collection of 3D routines for
...
result, then $a is assigned a reference to that. 
After this, the original $a is destroyed so there is no I<permanent>
