...
it outputs XS code (See L<perlxs>) but that is very close to
C).
...
have to tell PP by using the C<$PRIV> macro, i.e. you write
C<$PRIV(fd)> as in the example. Otherwise PP wouldn't know that the
...
therefore be left untouched by xsubpp (cf. L<perlxs> and L<perlxstut>
manpages).
...
Similar as the C<pp_add_exported> function works the C<pp_add_isa>
command. As the name already tells you C<pp_add_isa> adds its
arguments to the @ISA list so that you can say, e.g.
...
(if you don't know what that is check L<perlxs>). This is easily done
...
(csoelle@sghms.ac.uk). All rights reserved. Although destined for
