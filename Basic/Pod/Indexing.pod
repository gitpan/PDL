...
=item document perl level threading

=item threadids

=item update and correct description of slice

=item new functions in slice.pd (affine, lag, splitdim)

=item reworking of paragraph on explicit threading
...
routines (e.g. C<->index(...)>) result in physical piddles.
...
examples. It is the aptly named C<index> function, signature
...
mabe dataflow as well, see L<PDL::Dataflow> manpage). For further details
...
the basic ones defined in primitiv.pd
