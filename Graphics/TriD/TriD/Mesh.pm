...
#
# Use PDL::TriD::Surface for more general stuff.
#
...
sub get_boundingbox {
	...
	return PDL::TriD::BoundingBox->new( $foo->list() );
}
...
sub pdltotrianglemesh {
	...
	if($#{$pdl->{Dims}} != 1) { croak "Too many dimensions for PDL::GL::Mesh: $#{$pdl->{Dims}}  \n"; }
	my ($d0,$d1); my($x,$y);
