...
B<vsearch> performs a binary search for the values from C<$vals>
piddle in the ordered piddle C<$x>, returning indices into C<$x>.
It is a front end to a set of routines which differ in how matches are
determined and the meaning of the returned indices.
...
on a grid where the left bin edges are I<inclusive> of the bin.

=item C<insert_bin_exclusive>

invoke B<vsearch_bin_exclusive>, returning an index appropriate for binning
on a grid where the left bin edges are I<exclusive> of the bin.
