...
#define TIFFMAXCOL 65535L  /* maximal color that can be stored in an uint16 */
    if ( chv == (colorhist_vector) 0 )
	cht = (colorhash_table) 0;
    else
	{
	/* Make TIFF colormap. */
	  /* change in color scale behaviour by CS */
	  if (maxval > TIFFMAXCOL) /* only scale if necessary */
	    for ( i = 0; i < colors; ++i )
	      {
	    ...
	    for ( i = 0; i < colors; ++i )
	      {
		red[i] = (long) PPM_GETR( chv[i].color );
		grn[i] = (long) PPM_GETG( chv[i].color );
		blu[i] = (long) PPM_GETB( chv[i].color );
	      }
	  /* and also fill the remaining colors from color...(1<<bitspersample)
	     with zeroes; otherwise libtiff will later think this is a 16bit
	     lookup table if it contains random values > 256 (CS) */
	  {
	    long max;
	    max = 1L << bitspersample;
	    if (max > MAXCOLORS) max=MAXCOLORS;
	    for ( i = colors; i < max; ++i )
	      {
		red[i] = 0;
		grn[i] = 0;
		blu[i] = 0;
